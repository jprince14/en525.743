...
void initialize_encoder(struct encoder* lame_encoder) {
	...
	lame_set_in_samplerate(lame_encoder->lame, (48000 / 2));
...
void encoder_work(struct liquidobjects* dsp, struct encoder* lame_encoder) {
			...
			if (dsp->buffercounter > (20 * 8192)) {
				printf("dsp->buffercounter = %d\n", dsp->buffercounter);
				dsp->buffercounter = 0;

//		lame_encoder->mp3buffsize = lame_encode_buffer_interleaved_ieee_float(lame_encoder->lame,
//				(float *) dsp->buf_resamp, dsp->buffercounter, lame_encoder->mp3_buffer, 21*8192);
