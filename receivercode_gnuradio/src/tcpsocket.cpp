...
tcpsocket::tcpsocket() :
	...
	filter_fc = 0.15f;          // filter cutoff frequency
	filter_ft = 0.05f;			//filter transition
...
//			ofstream _filteredanddemodulatedfile;
			...
			freqdem_demodulate_block(fdem, reinterpret_cast<liquid_float_complex*>(&complexinputbuffer), buffercounter,
					demodoutput);
...
//		myfile.write((char*) &complexbuffer, sizeof(complexbuffer));
		...
		complexinputbuffer[buffercounter] = (scale * (_I - 127)) + (I * scale * (_Q - 127));
